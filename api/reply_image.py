...
Quest-reply image upload, end-to-end in Supabase Storage.

    POST /api/reply-image  (multipart, field name: "image")

Files land at `reply_images/<uploader_uid>/<ts>.<ext>`. RLS on
storage.objects requires the first folder segment to match the
uploader's auth.uid().

The endpoint returns the public URL — clients include it in the
`img_url` field when POSTing to /api/quest-replies.
"""
import time
...
class ReplyImageUploadHandler(View):
    ...
    def post(self, request) -> JsonResponse:
        ...
        if status >= 400:
            ...
            if (status == 404
                ...
                return json_error(
                    f"reply_images bucket missing or unwritable: {msg}. "
